...
def back_prop(param_w, param_b, param_a, layer, y_dat, Lambda):
	theta = [0.] * layer
	_w = [0.] * layer
	_b = [0.] * layer
	w_sum = 0
...
def train(args, x_dat, y_dat):
	...
	for iters in range(args.iteration):
		...
		for i, dat in enumerate(batch_x):
			...
			_w, _b = back_prop(param_w, param_b, param_a, layer, batch_y[i], Lambda) 
	...
	args.output_file = 'eta['+str(eta)+'],'+'lambda['+str(Lambda)+'],'+'Ein['+str(ein)+'].csv'
